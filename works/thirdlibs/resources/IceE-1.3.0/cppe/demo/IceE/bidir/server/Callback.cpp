...
static const ::std::string __Demo__CallbackSender__addClient_name = "addClient";
...
{
    ::std::pair< ::std::string*, ::std::string*> r = ::std::equal_range(__Demo__CallbackSender_all, __Demo__CallbackSender_all + 5, current.operation);
    if(r.first == r.second)
    {
        throw Ice::OperationNotExistException(__FILE__, __LINE__, current.id, current.facet, current.operation);
    }
...
IceProxy::Demo::CallbackSender::addClient(const ::Ice::Identity& ident, const ::Ice::Context* __ctx)
{
    ...
    while(true)
    {
        ...
        {
            __handler = __getRequestHandler();
            ::IceInternal::Outgoing __outS(__handler.get(), _reference.get(), __Demo__CallbackSender__addClient_name, ::Ice::Normal, __ctx);
            try
            {
                ::IceInternal::BasicStream* __os = __outS.os();
                ident.__write(__os);
